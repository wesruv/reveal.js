...
It will automatically make equal width and height columns that add up to 100% of the `.flex-wrapper`'s width.

It assumes that all direct descendants of `flex-wrapper` are a column, no special classes or markup required.
...
The default `flex-wrapper` styles are:
```css
display: flex;
align-items: stretch;
flex-wrap: wrap;
